...
function get_company_logo($uri = '', $href_class = '', $type = '')
{
    ...
    if ($company_logo != '') {
        $logo = '<a href="' . $logoURL . '" class="header-brand logo img-responsive ' . ($href_class != '' ? ' ' . $href_class : '') . '">
        <img width="auto" style="min-height: 60px; height: 60px"  src="' . base_url('uploads/company/' . $company_logo) . '" class="img-responsive" alt="' . html_escape($company_name) . '">
        </a>';
    } elseif ($company_name != '') {
        $logo = '<a href="' . $logoURL . '" class="header-brand ' . $href_class . ' logo logo-text">' . $company_name . '</a>';
    } else {
        $logo = '';
    }
