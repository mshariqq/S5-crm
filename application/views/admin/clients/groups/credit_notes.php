...
<?php if(isset($client)){ ?>
	...
	<?php if(has_permission('credit_notes','','create')){ ?>
		<a href="<?php echo admin_url('credit_notes/credit_note?customer_id='.$client->userid); ?>" class="btn btn-primary mbot25<?php if($client->active == 0){echo ' disabled';} ?>">
			<?php echo _l('new_credit_note'); ?>
		</a>
	<?php } ?>
	<?php if(has_permission('credit_notes','','view') || has_permission('credit_notes','','view_own')){ ?>
		<a href="#" class="btn btn-primary mbot25" data-toggle="modal" data-target="#client_zip_credit_notes"><?php echo _l('zip_credit_notes'); ?></a>
	<?php } ?>
	<?php
