...
foreach ($startedTimers as $timer) {
    ...
    if ($timer['task_id'] != '0') {
        ...
        $data .= '<button type=\'button\' onclick=\'timer_action(this,'.$timer['task_id'].','.$timer['id'].');\' class=\'btn btn-primary btn-xs\'>'._l('save').'</button>" ';
...
if ($noTimersWithoutTask
    ...
    echo '<button class="mtop15 text-start text-left btn btn-success started-timers-button top-dropdown-btn" onclick="timer_action(this,0); return false;"><i class="fa fa-clock-o"></i> '._l('task_start_timer').'</button>';
}

if (is_admin()) {
    echo '<div class="text-left text-start mtop15 view-all-timesheets">';
    echo '<a class="btn btn-primary" href="'.admin_url('staff/timesheets?view=all').'">'._l('view_members_timesheets').' <i class="fa fa-history"></i> </a>';
    echo '</div>';
}
